...
A reference implementation (v0.1 target): **a ~200-line middleware** (Python/FastAPI) that fetches a manifest, classifies inbound requests by agent identity header, and enforces `allow/deny/require_approval`. Enough to demo, small enough to audit.  
